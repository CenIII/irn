...
class PathIndexClsbd:
	...
	def __init__(self, radius=5):
		...
		self.path_list_by_length, self.path_dst, self.max_path_len = self.get_all_dir_paths(self.radius)
	...
	def get_all_dir_paths(self, max_radius=5):
		...
		for y in range(-max_radius + 1, max_radius):
		...
		for dir in search_dirs:
			...
			max_len = path_length if path_length>max_len else max_len
		...
		return path_list_by_length, path_destinations, max_len
	
	def path_index_to_tensor(self, training=False):
		# max_len = len(self.path_list_by_length)
		...
		return kernel_paths, mask  # [19,19,24,2]
