...
def run(args):
	...
	for ep in range(ep_start, args.cam_num_epoches):
		...
		if rt_key == 'model':
			...
			model.load_state_dict(torch.load('exp/deeplabv2_res101/sess/res50_cam_8_651raw.pth'), strict=False)
			...
			model_optimizer.set_start_step(661)
			while True:
			...
			model.load_state_dict(torch.load('exp/deeplabv2_res101/sess/res50_cam_6.pth'), strict=False)
			clsbd = getattr(importlib.import_module(args.irn_network), 'Net')()
			clsbd.load_state_dict(torch.load('exp/deeplabv2_res101/sess/res50_clsbd_7.pth'), strict=False)
